...
export async function mapCandidateToDtoAsync(
  ...
  return {
    ...
    yearsExperience: candidate.yearsExperience != null ? Number(candidate.yearsExperience) : null,
    ...
    skills: (candidate.skills ?? []).map((skill) => ({
      ...
      yearsExperience: skill.yearsExperience != null ? Number(skill.yearsExperience) : null,
...
export async function mapCandidateToListItemAsync(
  ...
  return {
    ...
    availableFrom: candidate.availableFrom?.toISOString().slice(0, 10) ?? null,
    timezoneOverlap: candidate.timezoneOverlap ?? candidate.timezone ?? null,
