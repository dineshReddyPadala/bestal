...
from app.resume.exceptions import InvalidAIResponseError, OpenAIRequestError
from app.resume.services.response_formatter import ResponseFormatter
...
class ResumeAnalysisAgent:
    ...
    def analyze(self, prompt: str, job_id: str):
        try:
        ...
        return self._formatter.parse(final_state["raw_response"], job_id=final_state["job_id"])
    ...
    def _validate_json(self, state: AgentState) -> AgentState:
        try:
            self._formatter.parse(state["raw_response"], job_id=state["job_id"])
