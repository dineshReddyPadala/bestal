...
RESPONSE_SCHEMA = {
    ...
    "skills": [
        {
            "name": "string — ONE short skill label only, max 60 chars (e.g. Python, React, AWS). Never a sentence or comma-list",
...
=== SKILL NAME RULES (critical — skills.name is stored in a 150-char DB field) ===
Each entry in `skills` is ONE discrete skill. The `name` field MUST be a short, searchable label.

Hard constraints for skills[].name:
- Maximum 60 characters (hard limit). Prefer under 40.
- Exactly ONE skill per array item — never combine multiple skills in one name.
- No commas, semicolons, pipes, or "and" joining multiple tools/technologies.
- No sentences, bullet fragments, proficiency notes, or years-of-experience text inside the name.
- Use the canonical short form recruiters search for (tool, language, framework, platform, or practice).

Good examples:
  "Python", "React", "TypeScript", "AWS", "PostgreSQL", "Docker", "Kubernetes",
  "Apache Spark", "TensorFlow", "System Design", "REST APIs", "CI/CD"

Bad examples (NEVER do this):
  "Python, Pandas, NumPy, Scikit-learn, TensorFlow"  ← split into separate skills
  "Strong experience in machine learning and data pipelines using Spark"  ← too long / sentence
  "Java (8+ years) / Spring Boot / Microservices"  ← put years in yearsExperience, split tools
  "Frontend: React, Redux, Next.js, Tailwind CSS"  ← split into separate skills
...
- Prefer 8–20 distinct skills covering the candidate's real stack; skip soft fluff ("team player", "hardworking").
...
- If the resume lists a long comma-separated skills line, split each token into its own skills[] entry.
