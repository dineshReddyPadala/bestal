...
export function SchemaFieldGrid({ fields, columns = 2, className, title }: SchemaFieldGridProps) {
  ...
  return (
    <div className={className}>
      ...
      <dl className={cn('grid gap-x-8 gap-y-4', gridClass)}>
        {visible.map((field) => (
...
export function candidateUserFields(c: import('@bestal/mock-data').SchemaCandidate): SchemaFieldDef[] {
  return [
    ...
    { key: 'linkedinUrl', label: 'LinkedIn', value: c.linkedinUrl, format: 'link' },
  ];
}
...
/** @deprecated Use candidateUserFields + candidateSystemFields */
export function candidateToFields(c: import('@bestal/mock-data').SchemaCandidate): SchemaFieldDef[] {
  return [...candidateUserFields(c), ...candidateSystemFields(c)];
}

export function clientUserFields(c: import('@bestal/mock-data').SchemaClient): SchemaFieldDef[] {
  return [
...
export function clientSystemFields(c: import('@bestal/mock-data').SchemaClient): SchemaFieldDef[] {
  return [
    ...
    { key: 'totalSpend', label: 'Total Spend', value: c.totalSpend, format: 'currency', currency: 'USD' },
    { key: 'createdAt', label: 'Created At', value: c.createdAt, format: 'datetime' },
    { key: 'updatedAt', label: 'Updated At', value: c.updatedAt, format: 'datetime' },
    { key: 'deletedAt', label: 'Deleted At', value: c.deletedAt, format: 'datetime' },
  ];
...
/** @deprecated Use clientUserFields + clientSystemFields */
export function clientToFields(c: import('@bestal/mock-data').SchemaClient): SchemaFieldDef[] {
  return [...clientUserFields(c), ...clientSystemFields(c)];
}
